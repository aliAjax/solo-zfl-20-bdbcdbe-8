...
7. **幂等**:`plan`/`replan` 必须带 `requestId`,同一请求号只生效一次,重复提交返回首次结果(`duplicated=true`);请求号跨操作类型复用返回 409。
8. **并发**:同一时刻只允许一个排期写操作,并发请求直接返回 `409 {conflict:true}`,不改数据。
...
- `404` 资源不存在 / 接口不存在
- `409` 并发排期冲突(`conflict:true`),或请求号被其他操作占用
